...
export class LngLat {
  ...
  toWGS84DMM(precision = 3): WGS84DMMCoordinate {
    return {
      ...
      latitudeMinutes: parseFloat(
        ((Math.abs(this.latitude) % 1) * 60).toFixed(precision),
      ),
      latDirection: this.latitude >= 0 ? 'N' : 'S',
      longitudeDegrees: Math.floor(Math.abs(this.longitude)),
      longitudeMinutes: parseFloat(
        ((Math.abs(this.longitude) % 1) * 60).toFixed(precision),
      ),
      lonDirection: this.longitude >= 0 ? 'E' : 'W',
    };
  }
  ...
  toWGS84DMS(precision = 2): WGS84DMSCoordinate {
    return {
      ...
      latitudeSeconds: parseFloat(
        ((((Math.abs(this.latitude) % 1) * 60) % 1) * 60).toFixed(precision),
      ),
      latDirection: this.latitude >= 0 ? 'N' : 'S',
      longitudeDegrees: Math.floor(Math.abs(this.longitude)),
      longitudeMinutes: Math.floor((Math.abs(this.longitude) % 1) * 60),
      longitudeSeconds: parseFloat(
        ((((Math.abs(this.longitude) % 1) * 60) % 1) * 60).toFixed(precision),
      ),
      lonDirection: this.longitude >= 0 ? 'E' : 'W',
    };
  }
