...
export class MapConfig {
  private unitIconSize = parseInt(
    localStorage.getItem('unit_icon_size') || '50',
  );
  private excludeStatuses = JSON.parse(
    localStorage.getItem('exclude_statuses') || '[6]',
  ) as number[];
  private provider = new LocalStorageProvider();
  private hideUnitsAfterPositionUpdate = parseInt(
    localStorage.getItem('hide_units_after_position_update') || '21600',
  ); // 6 hours in seconds
  ...
  public setExcludeStatuses(excludeStatuses: number[]): void {
    void this.provider.setItem(
      'exclude_statuses',
      JSON.stringify(excludeStatuses),
    );
  ...
  public setHideUnitsAfterPositionUpdate(seconds: number): void {
    void this.provider.setItem(
      'hide_units_after_position_update',
      seconds.toString(),
    );
