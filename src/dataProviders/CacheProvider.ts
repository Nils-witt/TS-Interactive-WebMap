...
class CacheProvider {
  ...
  async getOverlayCacheState(
    ...
    const cache = await caches.open(
      `overlay-${overlay.getId()}_${overlay.getLayerVersion()}`,
    );
  ...
  async cacheVector(
    ...
    const styleJson = (await style.json()) as {
    ...
    for (const sourceKey of Object.keys(styleJson.sources)) {
      const source = styleJson.sources[sourceKey] as {
        type: string;
        url: string;
      };
    ...
    for (let i = 0; i < urls.length; i = i + 10) {
      await Promise.all([
        ...urls
          .slice(i, i + 10)
          .map((tileUrl) =>
            this.cacheTile(tileUrl, cache, apiProvider.getToken()),
          ),
      ]);
    }
  }
  ...
  async cacheVectorForOverlays(apiProvider: ApiProvider): Promise<void> {
    const overlays = Array.from(
      DataProvider.getInstance().getAllMapOverlays().values(),
    );
   ...
   */
  async cacheVectorForOverlay(
    overlay: MapOverlay,
    mapStyle: MapBaseLayer,
    apiProvider: ApiProvider,
  ): Promise<void> {
    ...
    for (const z of Object.keys(pendingTiles).map((k) => parseInt(k, 10))) {
      for (const x of Object.keys(pendingTiles[z]).map((k) =>
        parseInt(k, 10),
      )) {
