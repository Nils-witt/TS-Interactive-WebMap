...
export class ApiProvider implements StorageInterface {
  ...
  private notifyListeners(
    event: ApiProviderEventTypes,
    data: { message: string },
  ): void {
    GlobalEventHandler.getInstance().emit(
      event,
      new ApiProviderEvent(event, data),
    );
  }
  ...
  public async testLogin(): Promise<void> {
    ...
    try {
      ...
      if (res.status == 401) {
        this.notifyListeners(ApiProviderEventTypes.UNAUTHORIZED, {
  ...
  public async login(username: string, password: string): Promise<string> {
    ...
    try {
      const res = await fetch(url, requestOptions);
      if (res.ok) {
        const data: { token: string; userId: string } = (await res.json()) as {
          token: string;
          userId: string;
        };
        return data.token;
  ...
  private async callApi(
    ...
    if (!response.ok) {
      if (response.status === 401) {
        this.notifyListeners(ApiProviderEventTypes.UNAUTHORIZED, {
          message: `Unauthorized access - check your token. ${response.status}`,
        });
      } else if (response.status === 403) {
        this.notifyListeners(ApiProviderEventTypes.FORBIDDEN, {
          message: `Forbidden - you don't have permission to access this resource. ${response.status}`,
        });
      }
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            ...
            data._embedded.unitDtoList == undefined
          ) {
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            data == null ||
            data._embedded == undefined ||
            data._embedded.mapGroupDtoList == undefined
          ) {
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            data == null ||
            data._embedded == undefined ||
            data._embedded.mapItemDtoList == undefined
          ) {
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            data == null ||
            data._embedded == undefined ||
            data._embedded.mapBaseLayerDtoList == undefined
          ) {
        ...
        this.fetchData(url)
          .then((raw) => {
            ...
            if (
              data == null ||
              data._embedded == undefined ||
              data._embedded.mapOverlayDtoList == undefined
            ) {
          ...
          .catch((e) => {
            return reject(
              new Error('Error fetching overlay layers', { cause: e }),
            );
          });
      } catch (error) {
        return reject(
          new Error('Error fetching overlay layers', { cause: error }),
        );
      }
    });
  }
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            data == null ||
            data._embedded == undefined ||
            data._embedded.photoDtoList == undefined
          ) {
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            data == null ||
            data._embedded == undefined ||
            data._embedded.missionGroupDtoList == undefined
          ) {
        ...
        .catch((e) => {
          return reject(
            new Error('Error fetching mission groups', { cause: e }),
          );
        });
    });
  }
      ...
      this.fetchData(url)
        .then((raw) => {
          ...
          if (
            data == null ||
            data._embedded == undefined ||
            data._embedded.userDtoList == undefined
          ) {
  ...
  loadAllNotifications(): Promise<Record<string, Notification>> {
    return Promise.reject(
      new Error('Method not implemented. (loadAllNotifications()))'),
    );
  }
  ...
  loadMissionGroup(id: string): Promise<MissionGroup> {
    return Promise.reject(
      new Error(`Method not implemented. (loadMissionGroup(${id})))`),
    );
  }

  loadUnit(id: string): Promise<Unit> {
    return Promise.reject(
      new Error(`Method not implemented. (loadUnit(${id}))`),
    );
  }

  loadMapGroup(id: string): Promise<MapGroup> {
    return Promise.reject(
      new Error(`Method not implemented. (loadMapGroup(${id}))`),
    );
  }

  loadMapItem(id: string): Promise<MapItem> {
    return Promise.reject(
      new Error(`Method not implemented. (loadMapItem(${id}))`),
    );
  }

  loadMapStyle(id: string): Promise<MapBaseLayer> {
    return Promise.reject(
      new Error(`Method not implemented. (loadMapStyle(${id}))`),
    );
  }

  loadMapOverlay(id: string): Promise<MapOverlay> {
    return Promise.reject(
      new Error(`Method not implemented. (loadMapOverlay(${id}))`),
    );
  }

  loadPhoto(id: string): Promise<Photo> {
    return Promise.reject(
      new Error(`Method not implemented. (loadPhoto(${id}))`),
    );
  }

  loadUser(id: string): Promise<User> {
    return Promise.reject(
      new Error(`Method not implemented. (loadUser(${id}))`),
    );
  }

  loadNotification(id: string): Promise<Notification> {
    return Promise.reject(
      new Error(`Method not implemented. (loadNotification(${id}))`),
    );
  }
  ...
  replaceAllUnits(units: Unit[]): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. replaceAllUnits(${units.length})`),
    );
  }
  ...
  replaceAllMapGroups(mapGroups: MapGroup[]): Promise<void> {
    return Promise.reject(
      new Error(
        `Method not implemented. replaceAllMapGroups(${mapGroups.length})`,
      ),
    );
  }
  ...
  replaceAllMapItems(mapItems: MapItem[]): Promise<void> {
    return Promise.reject(
      new Error(
        `Method not implemented. replaceAllMapItems(${mapItems.length})`,
      ),
    );
  }
  ...
  replaceAllMapStyles(mapStyles: MapBaseLayer[]): Promise<void> {
    return Promise.reject(
      new Error(
        `Method not implemented. replaceAllMapStyles(${mapStyles.length})`,
      ),
    );
  }
  ...
  replaceAllMapOverlays(mapOverlays: MapOverlay[]): Promise<void> {
    return Promise.reject(
      new Error(
        `Method not implemented. replaceAllMapOverlays(${mapOverlays.length})`,
      ),
    );
  }
  ...
  replaceAllPhotos(photos: Photo[]): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. replaceAllPhotos(${photos.length})`),
    );
  }

  replaceAllUsers(users: User[]): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. replaceAllUsers(${users.length})`),
    );
  }
  ...
  replaceAllMissionGroups(missionGroups: MissionGroup[]): Promise<void> {
    return Promise.reject(
      new Error(
        `Method not implemented. replaceAllMissionGroups(${missionGroups.length})`,
      ),
    );
  }
  ...
  replaceAllNotifications(notifications: Notification[]): Promise<void> {
    return Promise.reject(
      new Error(
        `Method not implemented. (replaceAllNotifications(${notifications.length}))`,
      ),
    );
  }
  ...
  saveMapGroup(mapGroup: MapGroup): Promise<MapGroup> {
    return Promise.reject(
      new Error(`Method not implemented. saveMapGroup(${mapGroup.getId()})`),
    );
  }
  ...
  saveMapStyle(mapStyle: MapBaseLayer): Promise<MapBaseLayer> {
    return Promise.reject(
      new Error(`Method not implemented. saveMapStyle(${mapStyle.getId()})`),
    );
  }
  ...
  saveMapOverlay(mapOverlay: MapOverlay): Promise<MapOverlay> {
    return Promise.reject(
      new Error(
        `Method not implemented. saveMapOverlay(${mapOverlay.getId()})`,
      ),
    );
  }
  ...
  savePhoto(photo: Photo): Promise<Photo> {
    return new Promise<Photo>((resolve) => {
      ...
      void this.callApi(url, 'PATCH', new Headers(), body).then((data) => {
  ...
  saveUser(user: User): Promise<User> {
    return Promise.reject(
      new Error(`Method not implemented. saveUser(${user.getId()})`),
    );
  }
  ...
  saveMissionGroup(missionGroup: MissionGroup): Promise<MissionGroup> {
    return new Promise<MissionGroup>((resolve, reject) => {
      ...
      const url =
        this.apiUrl +
        '/missiongroups' +
        (missionGroup.getId() ? '/' + missionGroup.getId() : '');

      this.callApi(
        url,
        missionGroup.getId() ? 'PUT' : 'POST',
        new Headers(),
        data,
      )
        .then((res) => {
        ...
        .catch((error) => {
          return reject(
            new Error('Failed to save mission group', { cause: error }),
          );
        });
    });
  }
  ...
  saveNotification(notification: Notification): Promise<Notification> {
    return Promise.reject(
      new Error(
        `Method not implemented. saveNotification(${notification.getId()})`,
      ),
    );
  }
  ...
  deleteUnit(id: string): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. deleteUnit(${id})`),
    );
  }

  deleteMapGroup(id: string): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. deleteMapGroup(${id})`),
    );
  }
  ...
  deleteMapItem(id: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      void this.callApi(url, 'DELETE', new Headers())
        ...
        .catch((error) => {
          return reject(
            new Error('Failed to delete map item', { cause: error }),
          );
        });
    });
  }
  deleteMapStyle(id: string): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. deleteMapStyle(${id})`),
    );
  }

  deleteMapOverlay(id: string): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. deleteMapOverlay(${id})`),
    );
  }
  ...
  deleteUser(id: string): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. deleteUser(${id})`),
    );
  }
  ...
  deleteMissionGroup(id: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      void this.callApi(url, 'DELETE', new Headers())
        ...
        .catch((error) => {
          return reject(
            new Error('Failed to delete mission group', { cause: error }),
          );
        });
    });
  }
  ...
  deleteNotification(id: string): Promise<void> {
    return Promise.reject(
      new Error(`Method not implemented. deleteNotification(${id})`),
    );
  }
  ...
  public async getMapOverlayTiles(overlay: MapOverlay): Promise<string[]> {
    ...
    if (overlay.getUrl().startsWith('http')) {
      url = new URL(
        overlay.getUrl().substring(0, overlay.getUrl().search('{z}')),
      );
    } else {
      url = new URL(
        overlay.getUrl().substring(0, overlay.getUrl().search('{z}')),
    ...
    const response = await fetch(
      url.href + '/index.json?accesstoken=' + this.apiToken,
      {
        cache: 'no-store',
        headers: {
          cache: 'no-store',
