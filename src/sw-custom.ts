...
new BroadcastChannel('addVectorCacheUrl').addEventListener(
  'message',
  (e: { data: { url: string; id: string } }) => {
...
new BroadcastChannel('addOverlayCacheUrl').addEventListener(
  'message',
  (e: { data: { url: string; id: string } }) => {
...
new BroadcastChannel('removeOtherOverlayCaches').addEventListener(
  'message',
  (e: { data: { version: string; id: string } }) => {
    const id = e.data.id;
    ...
    void caches.keys().then((keys) => {
      keys.forEach((key) => {
        if (
          key.startsWith(`overlay-${id}`) &&
          key != `overlay-${id}_${version}`
        ) {
...
function overlayRegex(url: string): RegExp {
  return new RegExp(
    `^${url.replaceAll('.', '\\.').replace('{x}', '\\d+').replace('{y}', '\\d+').replace('{z}', '\\d+')}(\\?.*)?$`,
  );
}
...
sw.addEventListener('fetch', (event) => {
  ...
  const [useCacheName, networkFirst, useCache, putMissingInCache] =
    getCacheName(url);
  if (useCache) {
    if (networkFirst) {
      event.respondWith(
      ...
      event.respondWith(
        caches.open(useCacheName).then((cache) => {
          return cache
            .match(transformCacheUrl(useCacheName, event.request.url))
            .then((cachedResponse) => {
              if (cachedResponse) {
