...
export function MapComponent() {
  ...
  const [qItem, setQItem] = useState<string | null>(
    searchParams.get('mapItemId'),
  );
  const [qGroup, setQGroup] = useState<string | null>(
  ...
  const [mapCenter, setMapCenter] = useState<[number, number]>(
    localStorage.getItem('mapCenter')
      ? (JSON.parse(
          localStorage.getItem('mapCenter') || '[7.1545,50.7438]',
        ) as [number, number])
      : [7.1545, 50.7438],
  );
      ...
      units.forEach((u) => {
        if (u.getId() === qUnit) {
          ...
          if (u.getPosition()) {
            setZoom(16);
            setMapCenter([
              u.getPosition()!.getLongitude(),
              u.getPosition()!.getLatitude(),
            ]);
          }
        }
      });
  ...
  return (
    <>
      ...
      {contextMenu && (
        ...
        >
          <button
            onClick={() => openCreateDialog(contextMenu.lng, contextMenu.lat)}
          >
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              !createName.trim() ||
              isNaN(parseFloat(createLat)) ||
              isNaN(parseFloat(createLng))
            }
          >
            Create
          </Button>
