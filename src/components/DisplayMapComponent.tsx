...
export function DisplayMapComponent({
  ...
  visibleUnitIds,
  showUnitsStatusBar,
}: DisplayMapComponentProps) {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [mapCenter, setMapCenter] = useState<[number, number]>(
    localStorage.getItem('mapCenterDisplay')
      ? (JSON.parse(
          localStorage.getItem('mapCenterDisplay') || '[7.1545,50.7438]',
        ) as [number, number])
      : [7.1545, 50.7438],
  );
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [zoom, setZoom] = useState<number>(
    localStorage.getItem('mapZoomDisplay')
      ? parseFloat(localStorage.getItem('mapZoomDisplay') || '10')
      : 10,
  );
  ...
  useEffect(() => {
    ...
    const activeSourceIds = new Set(
      sortedOverlays.map(
        (overlay) => `display-overlay-source-${overlay.getId()}`,
      ),
    );
    const activeLayerIds = new Set(
      sortedOverlays.map(
        (overlay) => `display-overlay-layer-${overlay.getId()}`,
      ),
    );
    ...
    const existingSourceIds = Object.keys(style.sources).filter((id) =>
      id.startsWith('display-overlay-source-'),
    );
    ...
    for (const overlay of sortedOverlays) {
      ...
      map.setLayoutProperty(
        layerId,
        'visibility',
        visibleOverlays.has(overlay.getId()) ? 'visible' : 'none',
      );
    }
  ...
  const updateView = (e: ViewStateChangeEvent) => {
    localStorage.setItem(
      'mapCenterDisplay',
      JSON.stringify([e.viewState.longitude, e.viewState.latitude]),
    );
  ...
  return (
    <div className="display-map-component">
      ...
      >
        <UnitDisplay
          showOnly={Array.from(visibleUnits)}
          showAlways={true}
          iconSize={75}
          showStatusBar={showUnitsStatusBar || false}
        />
      </MapLibreMap>
