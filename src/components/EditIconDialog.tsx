...
import {
    ...
    grundzeichen,
    type GrundzeichenId,
    organisationen, type OrganisationId,
    symbole,
    type SymbolId,
...
export function EditIconDialog(props: EditIconDialogProps) {
    ...
    const [grundzeichenId, setGrundzeichenId] = useState<GrundzeichenId>('fahrzeug');
    ...
    const [verwaltungsStufeId, setVerwaltungsStufeId] = useState<VerwaltungsstufeId | ''>('');
    ...
    const getTaktischesZeichen: () => TaktischesZeichen = () => {
        return {
            ...
            verwaltungsstufe: verwaltungsStufeId != '' ? verwaltungsStufeId : undefined,
    ...
    }, [grundzeichenId, fachaufgabeId, organisationId, einheitId, verwaltungsStufeId, symbolId, name, text, orgName, typ]);
    ...
    return (
        <Dialog open={props.open} onClose={props.onClose} fullWidth maxWidth="xs">
            <DialogTitle>Edit Unit symbol: {props.unitName}<br/>
                <img src={imgSrc} style={{maxHeight: '80px'}}/>
            </DialogTitle>
            <DialogContent>

                <Box sx={{display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important'}}>
                    <FormControl fullWidth size="small">
                        ...
                        >
                            {grundzeichen.map((gz) => {
                                return <MenuItem key={gz.id} value={gz.id}>{gz.label}</MenuItem>
                            })}
                        </Select>
                    </FormControl>
                    <FormControl fullWidth size="small">
                        ...
                        >
                            ...
                            {fachaufgaben.map((fa) => {
                                return <MenuItem key={fa.id} value={fa.id}>{fa.label}</MenuItem>
                            })}
