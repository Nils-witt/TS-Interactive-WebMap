...
export function SettingsPage(): JSX.Element {
  ...
  const [hideUnitsAfterPositionUpdate, setHideUnitsAfterPositionUpdate] =
    useState<number>(21600);
  ...
  useEffect(() => {
    ...
    setHideUnitsAfterPositionUpdate(
  ...
  const resetPWA = async () => {
    ...
    for (const name of names) {
      ...
      await new Promise((resolve, reject) => {
        ...
        req.onerror = () =>
          reject(
            Object.assign(new Error(`Failed to delete IndexedDB: ${name}`), {
              name,
              event: req.error,
            }),
          );
      ...
      <Container maxWidth="md">
        ...
        <Paper sx={{ p: 2, mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Typography>
                Hide units after no position update (seconds):
              </Typography>
              <Input
                type="number"
