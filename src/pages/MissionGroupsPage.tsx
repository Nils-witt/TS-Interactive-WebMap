...
export function MissionGroupsPage(): JSX.Element {
  ...
  const [assignedUnitsMissionGroup, setAssignedUnitsMissionGroup] =
    useState<MissionGroup | null>(null);
  const [assignedMapGroupsMissionGroup, setAssignedMapGroupsMissionGroup] =
    useState<MissionGroup | null>(null);

  const [editingMissionGroup, setEditingMissionGroup] =
    useState<MissionGroup | null>(null);
  const [deletingMissionGroup, setDeletingMissionGroup] =
    useState<MissionGroup | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    if (nameFilter.trim()) {
      ...
      result = result.filter((mg) =>
        mg.getName().toLowerCase().includes(lower),
      );
    }
    ...
    return [...result].sort((a, b) => {
      switch (sortField) {
        ...
        case 'startTime':
          return (
            a.getStartTime().getTime() - b.getStartTime().getTime() * sortDir
          );
        case 'endTime': {
        ...
        case 'mapGroups':
          return (
            (a.getMapGroupIds().length - b.getMapGroupIds().length) * sortDir
          );
        default:
          return 0;
      }
    ...
    <Box sx={{ overflowY: 'auto', height: '100%', py: 3 }}>
      <Container maxWidth="xl">
        ...
        >
          ...
          <Chip
            label={`${filtered.length} / ${missionGroups.length}`}
            variant="outlined"
            size="small"
          />
          <TextField
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((mg) => (
                  <TableRow key={mg.getId()} hover>
                    ...
                    <TableCell>
                      {formatDate(mg.getStartTime().getTime())}
                    </TableCell>
                    <TableCell>
                      {formatDate(mg.getEndTime()?.getTime() || null)}
                    </TableCell>
                      ...
                      <Chip
                        ...
                        onClick={
                          mg.getPermissions().includes('EDIT')
                            ? () => setAssignedUnitsMissionGroup(mg)
                            : undefined
                        }
                      />
                    </TableCell>
                    <TableCell align="center">
                      <Chip
                        ...
                        onClick={
                          mg.getPermissions().includes('EDIT')
                            ? () => setAssignedMapGroupsMissionGroup(mg)
                            : undefined
                        }
                      />
                    </TableCell>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            color="error"
            variant="contained"
            onClick={confirmDeleteMissionGroup}
          >
            Delete
          </Button>
        </DialogActions>
