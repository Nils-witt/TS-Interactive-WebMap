...
function OverlayTableRow({ overlay }: { overlay: MapOverlay }): JSX.Element {
  ...
  const downloadLayer = async () => {
    if (btnRef.current) {
      ...
      await CacheProvider.getInstance().cacheOverlay(
        overlay,
        apiProvider,
        btnRef.current,
      );
  ...
  useEffect(() => {
    void CacheProvider.getInstance()
      .getOverlayCacheState(overlay, apiProvider)
      .then((res) => {
        if (btnRef.current) {
  ...
  const downloadVectorTiles = async (overlay: MapOverlay) => {
    await CacheProvider.getInstance().cacheVectorForOverlay(
      overlay,
  ...
  return (
    <TableRow key={overlay.getId()} hover>
      ...
      <TableCell align="right">
        {(overlay.getOpacity() * 100).toFixed(0)} %
      </TableCell>
      <TableCell align="right">
        <Input
          type="number"
      ...
      <TableCell align="center">
        <ButtonGroup size="small">
          <Button ref={btnRef} onClick={() => void downloadLayer()}>
            Download
          </Button>
          <Button onClick={() => void clearCacheOverlay(overlay)}>
            Delete Offline Cache
          </Button>
          <Button onClick={() => void downloadVectorTiles(overlay)}>
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Chip
          label={`${filtered.length} / ${overlays.length}`}
          size="small"
          variant="outlined"
        />
      </Box>
        ...
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell
                sortDirection={sortField === 'url' ? sortOrder : false}
              >
                <TableSortLabel
              ...
              <TableCell
                sortDirection={sortField === 'opacity' ? sortOrder : false}
                align="right"
              >
                <TableSortLabel
              ...
              <TableCell
                sortDirection={sortField === 'order' ? sortOrder : false}
                align="right"
              >
                <TableSortLabel
