...
export function PhotoPage(): JSX.Element {
  ...
  const openEditDialog = (photo: Photo, e: React.MouseEvent) => {
    ...
    setEditLatitude(
      photo.getPosition() ? String(photo.getPosition()!.getLatitude()) : '',
    );
    setEditLongitude(
      photo.getPosition() ? String(photo.getPosition()!.getLongitude()) : '',
    );
  };
  ...
  const confirmDeletePhoto = () => {
    if (!editingPhoto || !editingPhoto.getId()) return;
    apiProvider
      .deletePhoto(editingPhoto.getId())
      .then(() => {
      ...
      position:
        ...
        editLongitude !== ''
          ? {
              ...
              timestamp:
                editingPhoto.getPosition()?.getTimestamp().toISOString() ??
  ...
  useEffect(() => {
    if (missionGroupFilter.trim() == '') {
      ...
      missionGroups.filter((mg) => mg.getId() === missionGroupFilter).length ===
      0
    ) {
      if (missionGroups.length > 0) {
        setMissionGroupFilter(missionGroups[0].getId());
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 2, pb: 2 }}>
        {photos.length === 0 ? (
          ...
          <ImageList variant="masonry" cols={cols} gap={8}>
            {photos
              .filter(
                (photo) => photo.getMissionGroupId() === missionGroupFilter,
              )
              .map((photo) => (
                <ImageListItem
      ...
      <Dialog open={confirmDelete} onClose={() => setConfirmDelete(false)}>
        ...
        <DialogContent>
          <Typography>
            ...
            <strong>{editingPhoto ? editingPhoto.getName() : ''}</strong>? This
            cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        {selected && (
          <>
            ...
            <DialogContent
              sx={{ p: 0, textAlign: 'center', backgroundColor: '#000' }}
            >
              <img
