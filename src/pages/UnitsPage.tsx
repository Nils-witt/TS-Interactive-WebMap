  ...
  const filtered = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      switch (sortField) {
        ...
        case 'group':
          return (
            (a.getGroupId() ?? '').localeCompare(b.getGroupId() ?? '') * sortDir
          );
        case 'latitude': {
    ...
    <Box sx={{ overflowY: 'auto', height: '100%', py: 3 }}>
      <Container maxWidth="lg">
        ...
        >
          ...
          <Chip
            label={`${filtered.length} / ${units.length}`}
            variant="outlined"
            size="small"
          />
          <TextField
          ...
          <FormControl size="small" sx={{ minWidth: 300 }}>
            <InputLabel id="units-status-filter-label">
              Status Filter
            </InputLabel>
            <Select
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((unit) => (
                  <UnitTableRow
                    key={unit.getId()}
                    unit={unit}
                    setEditStatusUnit={setEditStatusUnit}
      ...
      />
      <UnitStatusDialog
        open={editStatusUnit != null}
        unit={editStatusUnit}
        onClose={() => {
          setEditStatusUnit(null);
        }}
        onSave={updateUnitStatus}
      />
    </Box>
  );
}
    ...
    <TableRow key={unit.getId()} hover>
      ...
      <TableCell>
        <Typography variant="body2" color="text.secondary">
          {pos ? pos.getTimestamp().toLocaleString() : '—'}
        </Typography>
      </TableCell>
      ...
      <TableCell align="center">
        <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'center' }}>
          ...
          {unit.getPermissions().includes('EDIT') && (
            <Tooltip title="Edit">
              <IconButton size="small" onClick={() => openEditDialog(unit)}>
