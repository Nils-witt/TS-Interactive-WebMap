...
export function MapLocationPage(): JSX.Element {
  ...
  const filtered = useMemo(() => {
    ...
    if (nameFilter.trim()) {
      ...
      result = result.filter((item) =>
        item.getName().toLowerCase().includes(lower),
      );
    }
  ...
  const [bulkEditShowOnMap, setBulkEditShowOnMap] = useState<
    '' | 'true' | 'false'
  >('');
  ...
  const saveBulkEdit = () => {
    const selectedItems = items.filter(
      (item) => item.getId() !== null && selectedIds.has(item.getId()),
    );
  ...
  return (
    <>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
          ...
          {selectedIds.size > 0 && (
            ...
            >
              ...
              <Button variant="outlined" size="small" onClick={openBulkEdit}>
                Edit Selected
              </Button>
              <Button
                variant="contained"
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell
                  sortDirection={sortField === 'groupId' ? sortOrder : false}
                >
                  <TableSortLabel
                ...
                <TableCell
                  sortDirection={sortField === 'latitude' ? sortOrder : false}
                  align="right"
                >
                  <TableSortLabel
                ...
                <TableCell
                  sortDirection={sortField === 'longitude' ? sortOrder : false}
                  align="right"
                >
                  <TableSortLabel
                ...
                <TableCell
                  sortDirection={sortField === 'zoomLevel' ? sortOrder : false}
                  align="right"
                >
                  <TableSortLabel
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((item) => (
                  <TableRow
                    ...
                    hover
                    selected={
                      item.getId() !== null && selectedIds.has(item.getId())
                    }
                  >
                    <TableCell padding="checkbox">
                      <Checkbox
                    ...
                    <TableCell align="right">
                      {item.getLatitude().toFixed(6)}
                    </TableCell>
                    <TableCell align="right">
                      {item.getLongitude().toFixed(6)}
                    </TableCell>
      ...
      >
        ...
        >
          <Typography variant="body2" color="text.secondary">
            Only fields you change will be updated. Leave a field at "Keep
            current" to leave it unchanged.
          </Typography>
          ...
          <FormControl size="small" fullWidth>
            ...
            <Select
              ...
              label="Show on Map"
              onChange={(e) =>
                setBulkEditShowOnMap(e.target.value as '' | 'true' | 'false')
              }
            >
              <MenuItem value="">
                <em>Keep current</em>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ...
            selected item(s)? This cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={confirmDelete} onClose={() => setConfirmDelete(false)}>
        ...
        <DialogContent>
          <Typography>
            ...
            <strong>{editingItem?.getName()}</strong>? This cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              !editName.trim() ||
              isNaN(parseFloat(editLatitude)) ||
              isNaN(parseFloat(editLongitude))
            }
          >
            Save
          </Button>
