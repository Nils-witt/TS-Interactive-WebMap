...
export function DisplayPage() {
  ...
  const [selectedOverlayIds, setSelectedOverlayIds] = useState<string[]>(
    JSON.parse(
      localStorage.getItem('displayOverlaysShown') || '[]',
    ) as string[],
  );

  const units = useContext(UnitsContext);
  const [selectedUnitIds, setSelectedUnitIds] = useState<string[]>(
    JSON.parse(localStorage.getItem('displayUnitsShown') || '[]') as string[],
  );

  const [settingsDialogOpen, setSettingsDialogOpen] = useState(false);

  const [showUnitsStatusBar, setShowUnitsStatusBar] = useState<boolean>(
    () =>
      (JSON.parse(
        localStorage.getItem('displayShowUnitsStatusBar') || 'false',
      ) as boolean) || false,
  );

  useEffect(() => {
    localStorage.setItem(
      'displayOverlaysShown',
  ...
  useEffect(() => {
    localStorage.setItem(
      'displayShowUnitsStatusBar',
      JSON.stringify(showUnitsStatusBar),
    );
  }, [showUnitsStatusBar]);
  ...
  return (
    <div className="display-page">
      <Box sx={{ position: 'relative', width: '100%', height: '100%' }}>
        <DisplayMapComponent
          visibleOverlayIds={selectedOverlayIds}
          visibleUnitIds={selectedUnitIds}
