...
export function CachePage(): JSX.Element {
  ...
  const [selectedCacheName, setSelectedCacheName] = useState<string | null>(
    null,
  );
  const [selectedCacheEntries, setSelectedCacheEntries] = useState<
    CacheRequestEntry[]
  >([]);
  ...
  const refresh = () => {
    ...
    void loadCaches().then((data) => {
      ...
      if (
        ...
        !data.some((entry) => entry.name === selectedCacheName)
      ) {
      ...
      {selectedCacheName != null ? (
        <Paper sx={{ mt: 3, p: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
            ...
            <Chip
              label={`${selectedCacheEntries.length} requests`}
              size="small"
            />
          </Box>
