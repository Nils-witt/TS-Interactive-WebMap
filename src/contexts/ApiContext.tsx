...
export const ApiContext = createContext<ApiProvider>(new ApiProvider(DataProvider.getInstance().getApiUrl(), undefined));

export function ApiContextProvider({ children }: { children: ReactNode }): JSX.Element {

    const auth = useAuth();
    const databaseProvider = useContext(DataBaseContext);

    const [localApiToken, setLocalApiToken] = useState<string | undefined>(DataProvider.getInstance().getLocalApiToken());
    ...
    useEffect(() => {
        ...
        DataProvider.getInstance().on(DataProviderEventType.LOCAL_API_TOKEN_UPDATED, onChange);

        return () => {
            DataProvider.getInstance().off(DataProviderEventType.LOCAL_API_TOKEN_UPDATED, onChange);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (!apiToken || !apiUrl) {
            console.log("API token or URL not set, not starting WebSocket connection");
