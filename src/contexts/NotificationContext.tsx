...
export function NotificationsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const refresh = () =>
      setNotifications(Array.from(dp.getAllNotifications().values()));
    ...
    if (!databaseProvider) {
      console.error(
        'NotificationsProvider: DatabaseProvider not available in context',
      );
      return;
    }
    void databaseProvider
      ...
      .then((result) => {
        Object.values(result).forEach((notification) =>
          dp.addNotification(notification),
        );
        refresh();
      })
