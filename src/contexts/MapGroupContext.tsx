...
export function MapGroupProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!databaseProvider) {
      console.error(
        'MapGroupProvider: DatabaseProvider not available in context',
      );
      return;
    }
      ...
      .then((result) => {
        ...
        if (apiProvider.hasToken()) {
          apiProvider
            ...
            .catch((e) =>
              console.error('MapGroupContext: remote load failed', e),
            );
        }
      })
  ...
  return (
    <MapGroupContext.Provider value={mapGroups}>
      {children}
    </MapGroupContext.Provider>
  );
}
