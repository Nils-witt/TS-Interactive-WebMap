...
export function UnitRepresentation(
  props: UnitRepresentationProps,
): React.JSX.Element {
  const [show, setShow] = useState<boolean>(false);

  const recheckTimeOut = () => {
    if (
      Date.now() - props.unit.getPosition()!.getTimestamp().getTime() >=
      props.hideUnitsAfterPositionUpdate * 1000
    ) {
      setShow(false);
    }
  };
  ...
  return (
    <>
      {show && props.unit.getImgSrc() != '' ? (
        <Marker
          latitude={props.unit.getPosition()!.latitude}
          longitude={props.unit.getPosition()!.longitude}
        >
          <div
            style={{ width: props.iconSize > 10 ? props.iconSize : 75 }}
            className="unit-respresentation-container"
            onContextMenu={(e) =>
              props.unit.getPermissions().includes('EDIT') &&
              props.onContextMenu(
            ...
            {props.showStatusBar ? (
              <div>
                <Chip
                  ...
                  size="small"
                  color={
                    STATUS_COLORS[props.unit.getStatus() || 10] ?? 'default'
                  }
                />
              </div>
            ) : (
